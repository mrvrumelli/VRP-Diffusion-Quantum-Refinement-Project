...
spec = importlib.util.spec_from_file_location("evaluate_synthetic", root / "eval" / "evaluate_synthetic.py")
...
from vrp_diffusion_quantum.data.generate_cvrp import CVRPDataset
...
def test_evaluate_dataset(mock_solve: MagicMock) -> None:
    ...
    rows = evaluate_dataset(dataset, solver="pyvrp", time_limit=1.0, seed=0, reference_costs=reference_costs)
...
def test_evaluate_dataset_mismatched_references(mock_solve: MagicMock) -> None:
    ...
    with pytest.raises(ValueError, match="reference costs \\(1\\) do not match dataset instances \\(2\\)"):
        evaluate_dataset(dataset, reference_costs=[100.0])


def test_summarize() -> None:
    rows = [
        {"size": 20, "cost": 105.0, "gap": 5.0, "runtime": 0.5, "feasibility": True, "number_of_vehicles": 5},
        {"size": 20, "cost": 100.0, "gap": 0.0, "runtime": 0.3, "feasibility": True, "number_of_vehicles": 4},
        {"size": 50, "cost": 300.0, "gap": 2.0, "runtime": 1.0, "feasibility": False, "number_of_vehicles": 10},
    ]
